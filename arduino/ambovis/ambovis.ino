...
void loop() {
  ...
  if (!systemState.sleep_mode) {
    if (systemState.wake_up) {
      lcd.clear();
      init_display();
      display_lcd(menuState, systemState.vent_mode, sensorParams);    //TODO: Pass mech vent as argument in display
      tft.fillScreen(ILI9341_BLACK);
      systemState.wake_up = false;
    }
    ...
    if (time > lastReadSensor + TIME_SENSOR) {
      ...
      p_dpt -= verror + (float(menuState.p_trim) - 100.0) * 1e-3; //WITH TRIM
